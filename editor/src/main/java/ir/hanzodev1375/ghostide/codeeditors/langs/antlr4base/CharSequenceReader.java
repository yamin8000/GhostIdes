 ...
 * tnks for Android ide to fix read path
 * @since 1.4
 */
public class CharSequenceReader extends Reader implements Serializable {
   ...
   * reading at the CharSequence's length. If this field was an int instead, it would be initialized
   * to 0 when the CharSequenceReader is de-serialized, causing it to not return any characters at
   * all.
   *
   * @see #end()
   * @since 2.7
   ...
   *
   * <p>The start index is not strictly enforced to be within the bounds of the character sequence.
   * This allows the character sequence to grow or shrink in size without risking any {@link
   * IndexOutOfBoundsException} to be thrown. Instead, if the character sequence grows smaller than
   * the start index, this instance will act as if all characters have been read.
   *
   * @param charSequence The character sequence, may be {@code null}
   ...
   * Constructs a new instance with a portion of the specified character sequence.
   *
   * <p>The start and end indexes are not strictly enforced to be within the bounds of the character
   ...
   * smaller than the start index, this instance will act as if all characters have been read; if
   * the character sequence grows smaller than the end, this instance will use the actual character
   * sequence length.
   *
   * @param charSequence The character sequence, may be {@code null}
   * @param start The start index in the character sequence, inclusive
   * @param end The end index in the character sequence, exclusive
   * @throws IllegalArgumentException if the start index is negative, or if the end index is smaller
